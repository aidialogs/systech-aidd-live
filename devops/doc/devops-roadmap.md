...
### Состав работ
- Учесть результаты спринта D1
- Учесть что есть настроенный сервер с доступом по SSH по ключу (установлены все для запуска docker compose)
...
- Описать загрузку образов
- Описать запуск сервисов
- Описать запуск миграций базы данных
- Создать шаблон `.env.production` с описанием всех переменных
- Пройти вручную по инструкции
- Выполнить проверку работоспособности проекта на удаленном сервере
